...
object GoodsRemoteDataSource : GoodsDataSource {
    ...
    override fun newGetBookCategory(callback: (Categories) -> Unit) {
        api.newGetBookCategory().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                callback(it)
            },{})
    }
